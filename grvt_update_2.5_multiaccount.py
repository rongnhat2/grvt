...
class MarketMaker:
    ...
    def _refresh_close(self, ticker: dict, open_api: Dict[str, dict]):
        """
        Manage the close order that returns the position to flat after a fill.

        KEY RULE — only refresh when price improves:
          - Long (need to sell): only update if new ask is higher than current close price
          - Short (need to buy):  only update if new bid is lower than current close price
        If price moves against us, hold the order and wait — never chase it down.
        """
        if self.pos.is_flat:
            if self.close_mo:
        ...
        if self.pos.is_long:
            # Long position — sell to close at best_ask
            target_px = _round(best_ask if best_ask else mark + tick, tick, up=True)
        else:
            # Short position — buy to close at best_bid
            target_px = _round(best_bid if best_bid else mark - tick, tick, up=False)

        if self.close_mo:
            if self.close_mo.client_oid not in open_api:
                log.info(f"  Close order filled/gone ✓")
                self.close_mo = None
            else:
                # Only cancel and reprice if the new price is strictly better (more profitable to close)
                if self.pos.is_long:
                    price_improved = target_px > self.close_mo.price + tick
                else:
                    price_improved = target_px < self.close_mo.price - tick

                if price_improved:
                    log.info(f"  Close refresh (better price): {self.close_mo.price:.2f} → {target_px:.2f}")
                    self._cancel(self.close_mo, confirm=True)
                    self.close_mo = None
                # else: price same or worse — hold the order, do NOT cancel
